...
public:
	...
	Fraction_Public(const string num = "0", const unsigned int frac = 0) throw(const char*, Exception, Bad_exception)
	{
		if (frac == 100)
	...
	friend istream& operator >>(istream& in, Fraction_Public& f) throw(const char*);
